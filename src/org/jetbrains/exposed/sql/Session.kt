...
class Session (val db: Database, val connector: ()-> Connection): UserDataHolder() {
    ...
    val outerSession = Session.Companion.threadLocal.get()

    init {
        logger.addLogger(Log4jSqlLogger())
        Session.Companion.threadLocal.set(this)
    }
    ...
    fun <T> execBatch(body: Session.BatchContext.() -> T): T {
    ...
    fun <T> exec(stmt: String, args: List<Pair<ColumnType, Any?>> = listOf(), body: () -> T): T {
        return execBatch {
            ...
            val metricsId = db.metrics?.startEvent(com.rimmer.metrics.EventType.mysql, stmt)
            val result = body()
    ...
    fun close() {
        ...
        Session.Companion.threadLocal.set(outerSession)
        _connection?.close()
    }
    ...
    companion object {
        ...
        fun hasSession(): Boolean = Session.Companion.tryGet() != null

        fun tryGet(): Session? = Session.Companion.threadLocal.get()

        fun get(): Session {
            return Session.Companion.tryGet() ?: error("No session in context. Use transaction?")
        }
    }
}
