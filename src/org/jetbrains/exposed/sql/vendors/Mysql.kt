...
internal object MysqlDialect : VendorDialect() {
    ...
    override @Synchronized fun tableColumns(): Map<String, List<Pair<String, Boolean>>> {
        ...
        val rs = Session.get().connection.createStatement().executeQuery(
                "SELECT DISTINCT TABLE_NAME, COLUMN_NAME, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '${MysqlDialect.getDatabase()}'")
    ...
    override @Synchronized fun columnConstraints(vararg tables: Table): Map<Pair<String, String>, List<ForeignKeyConstraint>> {
        ...
        val rs = Session.get().connection.createStatement().executeQuery(
                "SELECT\n" +
                        ...
                        "WHERE ku.TABLE_SCHEMA = '${MysqlDialect.getDatabase()}' ${inTableList()}")
        ...
        val rs = Session.get().connection.createStatement().executeQuery(
                """SELECT ind.* from (
                        SELECT
                            ...
                            WHERE table_schema = '${MysqlDialect.getDatabase()}' and INDEX_NAME <> 'PRIMARY'
                            GROUP BY 1, 2) ind
                LEFT JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE kcu
                    on kcu.TABLE_NAME = ind.TABLE_NAME
                        and kcu.COLUMN_NAME = ind.columns
                        and TABLE_SCHEMA = '${MysqlDialect.getDatabase()}'
    ...
    override fun <T : String?> ExpressionWithColumnType<T>.match(pattern: String, mode: MatchMode?): Op<Boolean> = MysqlDialect.MATCH(this, pattern, mode ?: MysqlMatchMode.STRICT)
