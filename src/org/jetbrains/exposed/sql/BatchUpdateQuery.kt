...
class BatchUpdateQuery(val table: IdTable) {
    ...
    private fun execute(session: Session, set: Collection<Pair<EntityID, HashMap<Column<*>, Any?>>>): Int {
        ...
        return session.execBatch {
            val metricsId = session.db.metrics?.startEvent(com.rimmer.metrics.EventType.mysql, sqlText)
            val stmt = session.prepareStatement(sqlText)
            for ((id, d) in set) {
                log(sqlText, columns.map {it.columnType to d[it]} + (org.jetbrains.exposed.sql.IntegerColumnType() to id))

                val idx = stmt.fillParameters(columns, d)
                stmt.setLong(idx, id.value)
                stmt.addBatch()
            }
