...
public class EntityID(id: Long, val table: IdTable) {
    ...
    val value: Long get() {
        if (_value == -1L) {
            EntityCache.Companion.getOrCreate(Session.get()).flushInserts(table)
...
class Referrers<out Source: Entity>(val reference: Column<EntityID>, val factory: EntityClass<Source>, val cache: Boolean) {
    ...
    operator fun get(o: Entity): SizedIterable<Source> {
        ...
        return if (cache) EntityCache.Companion.getOrCreate(Session.get()).getOrPutReferrers(o, reference, query)  else query()
    }
}
...
class InnerTableLink<Target: Entity>(val table: Table,
    ...
    operator fun getValue(o: Entity, p: KProperty<*>): SizedIterable<Target> {
        ...
        return EntityCache.Companion.getOrCreate(Session.get()).getOrPutReferrers(o, sourceRefColumn, query)
    }
    ...
    operator fun setValue(o: Entity, p: KProperty<*>, value: SizedIterable<Target>) {
        ...
        with(Session.get()) {
            val entityCache = EntityCache.Companion.getOrCreate(Session.get())
            entityCache.flush()
...
open public class Entity(val id: EntityID) {
    ...
    operator fun <T> Column<T>.setValue(o: Entity, desc: KProperty<*>, value: T) {
        if (writeValues.containsKey<Any>(this) || _readValues == null || _readValues!![this] != value) {
            if (referee != null) {
                EntityCache.Companion.getOrCreate(Session.get()).clearReferrersCache()
            }
...
class EntityCache {
    ...
    fun flush(tables: Iterable<IdTable>) {
        val sorted = addDependencies(tables).toCollection(ArrayList())
        sorted.topoSort { a, b ->
            when {
                a == b -> 0
    ...
    fun flushInserts(table: IdTable) {
        inserts.remove(table)?.let {
            ...
            for ((entry, id) in it.zip(ids)) {
                entry.id._value = id
                EntityCache.Companion.getOrCreate(Session.get()).store(table, entry)
    ...
    companion object {
        ...
        fun getOrCreate(s: Session): EntityCache {
            return s.getOrCreate(EntityCache.Companion.key, EntityCache.Companion.newCache)
        }
    }
}
...
abstract public class EntityClass<out T: Entity>(val table: IdTable) {
    ...
    open protected fun warmCache(): EntityCache = EntityCache.Companion.getOrCreate(Session.get())
    ...
    public fun wrap(id: EntityID, row: ResultRow?, s: Session): T {
        val cache = EntityCache.Companion.getOrCreate(s)
