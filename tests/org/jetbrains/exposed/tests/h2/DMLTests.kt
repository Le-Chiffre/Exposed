package org.jetbrains.exposed.tests.h2
...
class DMLTests : DatabaseTestsBase() {
    ...
    @Test fun testExists01() {
        withCitiesAndUsers { cities, users, userData ->
            val r = users.select{org.jetbrains.exposed.sql.exists(userData.select((userData.user_id eq users.id) and (userData.comment like "%here%")))}.toList()
            assertEquals(1, r.size)
    ...
    @Test fun testExists02() {
        withCitiesAndUsers { cities, users, userData ->
            val r = users.select{org.jetbrains.exposed.sql.exists(userData.select((userData.user_id eq users.id) and ((userData.comment like "%here%") or (userData.comment like "%Sergey"))))}
    ...
    @Test fun testExists03() {
        withCitiesAndUsers { cities, users, userData ->
            val r = users.select{
                        org.jetbrains.exposed.sql.exists(userData.select((userData.user_id eq users.id) and (userData.comment like "%here%"))) or
                                org.jetbrains.exposed.sql.exists(userData.select((userData.user_id eq users.id) and (userData.comment like "%Sergey")))
            }
                    .orderBy(users.id).toList()
            assertEquals(2, r.size)
            assertEquals("Sergey", r[0][users.name])
    ...
    @Test fun testCalc02() {
        withCitiesAndUsers { cities, users, userData ->
            val sum = org.jetbrains.exposed.sql.Expression.build {
    ...
    @Test fun testCalc03() {
        withCitiesAndUsers { cities, users, userData ->
            val sum = org.jetbrains.exposed.sql.Expression.build {Sum(cities.id * 100 + userData.value / 10, IntegerColumnType())}
            val r = (users innerJoin userData innerJoin cities).slice(users.id, sum)
                    .selectAll().groupBy(users.id).orderBy(users.id).toList()
            assertEquals(2, r.size)
            assertEquals("eugene", r[0][users.id])
    ...
    @Test fun testInsertSelect02() {
        withCitiesAndUsers { cities, users, userData ->
            userData.insert(userData.slice(userData.user_id, userData.comment, org.jetbrains.exposed.sql.intParam(42)).selectAll())
    ...
    @Test fun testSelectCase01() {
        withCitiesAndUsers { cities, users, userData ->
            val field = org.jetbrains.exposed.sql.Expression.build {case().When(users.id eq "alex", org.jetbrains.exposed.sql.stringLiteral("11")).Else (org.jetbrains.exposed.sql.stringLiteral("22"))}
...
/*
    Test fun testInsert05() {
        ...
        withTables(t) {
            t.insert {
                it[n] = 42
                it[d] = org.jetbrains.exposed.sql.today()
                it[e] = DMLTestsData.E.ONE
                it[s] = stringThatNeedsEscaping
            }

            val row = t.selectAll().single()
            t.checkRow(row, 42, null, org.jetbrains.exposed.sql.today(), null, DMLTestsData.E.ONE, null, stringThatNeedsEscaping, null)
        }
    }
*/
